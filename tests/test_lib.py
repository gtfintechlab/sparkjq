...
class TestSLURMCluster:
    ...
    def mock_dependencies(self, mock_slurm_env, mock_subprocess):
        ...
        mock_spark_ctx = SparkContext(home="/opt/spark-3.5.0", version="3.5.0")
    ...
    def test_slurm_cluster_init_custom_params(self, mock_dependencies):
        ...
        SLURMCluster(port=8080, scratch_dir="/custom/scratch", log_dir="/custom/logs")

        # Verify calls with custom params
        mock_dependencies["get_slurm"].assert_called_once_with(port=8080, scratch="/custom/scratch")
